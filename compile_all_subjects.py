...
def compile_rois(data, out, xml, group):
    ...
    for roi in ["cort", "sub"]:
        ...
        for img in ['INV2', 'GluCEST']:
            # read in the case's ROI values for ROI roi
            roi_table = img + '-HarvardOxfordROI-' + roi + '-Measures.tsv'
            roi_table = pd.read_table(os.path.join(out, roi_table), delimiter='\t')
            ...
            roi_table.to_csv(os.path.join(out, 'all_subs_' + img + '_' + roi + '_rois.csv'))
...
def main():
    ...
    required.add_argument(
        "-x", "--xml",
        help="path to JHU-tracts.xml file",
        metavar="",
        required=True)
    required.add_argument(
        "-g", "--group",
    ...
    compile_rois(args.input, args.output, args.xml, args.group)
