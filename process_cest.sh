...
if [ -e $structural/$case/MNI_transforms/$case-INV2inMNI-1InverseWarp.nii.gz ]
then
...
then
  ...
  do
    /project/bbl_projects/apps/melliott/scripts/dicom2nifti.sh -u -r Y -F \
      $post/$case/$case-$seq.nii $pre/$case/S*${seq}2D/*dcm
  done
...
fslmaths $post/$case/$case-B0MAP.nii \
  -add 10 \
...
fslmaths $post/$case/$case-B1MAP.nii \
  -thr 0.3 \
...
fslmaths $post/$case/$case-B0B1CESTMAP.nii \
...
for roi in cort sub
do
  antsApplyTransforms -d 3 -r $structural/$case/${case}-INV2-hdbet_masked.nii.gz \
    -i $atlas/HarvardOxford/HarvardOxford-$roi-maxprob-thr25-1mm.nii.gz \
    -n MultiLabel -o $post/$case/atlases/${case}-HarvardOxford-$roi.nii.gz  \
    -t [$structural/$case/MNI_transforms/${case}-INV2inMNI-0GenericAffine.mat,1] \
    -t $structural/$case/MNI_transforms/${case}-INV2inMNI-1InverseWarp.nii.gz

  /project/bbl_projects/apps/melliott/scripts/extract_slice2.sh \
    -MultiLabel $post/$case/atlases/${case}-HarvardOxford-$roi.nii.gz \
    $post/$case/orig_data/$case-B0B1CESTMAP.nii \
    $post/$case/atlases/${case}-2d-HarvardOxford-$roi.nii

  gzip $post/$case/atlases/${case}-2d-HarvardOxford-$roi.nii

  fslmaths $post/$case/atlases/$case-2d-HarvardOxford-$roi.nii.gz \
    -mul $post/$case/$case-tissuemap-bin.nii.gz \
    $post/$case/atlases/$case-2d-HarvardOxford-$roi.nii.gz

  fslmaths $post/$case/atlases/$case-2d-HarvardOxford-$roi.nii.gz \
    -bin $post/$case/atlases/$case-2d-HarvardOxford-$roi-bin.nii
